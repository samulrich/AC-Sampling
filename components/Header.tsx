        ...
        <header className="mb-6 flex-shrink-0">
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-8">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-800 tracking-tight">AC Drillhole Sampling</h1>
                        <p className="text-slate-500 text-sm mt-1">A simple, graphical tool for logging drill samples.</p>
                    </div>
                    {view === 'logger' && (
                        <div className="flex items-center gap-3">
                            ...
                            >
                                {drillHoles.map(hole => (
                                    <option key={hole.uuid} value={hole.uuid}>{hole.holeId || 'New Hole'}</option>
                                ))}
                            </select>
                            <button
                                onClick={onAddHole}
                                className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-white bg-green-600 rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
                            >
                               <PlusIcon className="w-4 h-4" /> Add Hole
                            </button>
                        </div>
                    )}
                </div>
